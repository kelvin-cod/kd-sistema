...
$.ajax({
    ...
    $.each(response, function (i, item) {
        ...
        tblCategoria +=
            ...
            ')"  class="btn btn-info " >' +
            '<img src="https://quebramedos.github.io/kd-sistema/images/teste.svg" />' +
            '</button>' +
            '</td>';

        if (item.idCategoria == 500) {
            tblCategoria += '<td><button type="button"  class=" btn btn-dark">' +
...
function editarCategoria(_id) {
    let produto;
     idCategoria = _id;
    if (_id == 500) {
        return alert("Não Possivel Editar")
    }
