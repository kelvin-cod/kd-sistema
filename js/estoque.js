...
$.ajax({
    ...
    $.each(response, function (i, item) {
        ...
        if (item.idCategoria == 500) {
            tblCategoria += '<td><button type="button"  class=" btn btn-dark " data-toggle="tooltip" data-placement="top" title="Não Permitido">' +
...
function excluir(id, tipo) {
    ...
    var post_url = "https://kd-gerenciador.herokuapp.com/produtos/excluirum/";
...
$("#categoria_modal").click(() => {
    $("#botaoEnviarCategoria").html('');

    let botao = '<button type="button" class="btn btn-success ml-5 Font-weight-bold"' +
        ' onclick="enviarCategoria()"  name="button">  ' +
        '  <i class="fas fa-check"></i> Enviar </button>';
...
function editarCategoria(_id) {
    ...
    $("#DescricaoCategoria").val(produto.categoria);
    $('#Estatus option[value= ' + produto.idCategoria + ' ]').attr('selected', 'selected');
